...
@pytest.mark.asyncio
async def test_fetch_prices_retry_on_429():
    # 1. Настройка
    ...
    # 2. Выполнение
    with patch("asyncio.sleep", new_callable=AsyncMock):  # Ускоряем тест, пропуская реальную паузу
        result = await client.fetch_prices(mock_session, [123])

    # 3. Проверка
    assert result is not None
    assert mock_session.get.call_count == 2  # Убеждаемся, что было сделано 2 попытки
